...
---
Task ID: chapter-onboarding-finalization
Agent: main
Task: Complete the chapter onboarding workflow per user spec — image uploads in the form, shareable WhatsApp URL, per-chapter image scoping on /admin/images, and the "Approve & Provision" action that takes a submitted form and creates a real Chapter + ChapterSetting rows + email infrastructure + links the lead User to the new chapter with CHAPTER_ORGANIZER role. Default email logo must fall back to the chaptercore.md blueprint default when the lead doesn't supply one.

Work Log:
- Shared chapter onboarding form URL pattern with the user: `{NEXT_PUBLIC_SITE_URL || VERCEL_URL || https://aisalon.massapro.com}/chapter-onboarding/{TOKEN}` — already generated by the send-chapter-onboarding API at invite creation, surfaced as "Copy form URL" + "Open form" buttons in /admin/chapter-onboarding.
- Verified state of all 7 task items in the codebase (all were implemented in the prior session before context ran out):
  1. ✅ Image upload fields — chapter-onboarding-form.tsx uses ImageUploadField (drag-and-drop + click) for all 5 brand assets: favicon, loginHero, loginBanner, landingHero, emailLogo. Accepts JPG/PNG/WebP/GIF/AVIF, 8MB cap, client-side validation mirroring server-side rules.
  2. ✅ Upload endpoint — POST /api/chapter-onboarding/[token]/upload-image accepts multipart/form-data, validates token (rejects REVOKED/EXPIRED/SUBMITTED), stores under chapter-onboarding/<token>/ prefix (Vercel Blob in prod, /public/uploads/chapter-onboarding/<token>/ locally). Returns { ok, url, name, size, mimeType }.
  3. ✅ Shareable URL surfaced — send-chapter-onboarding API route returns formUrl in the JSON response (both success + SMTP-failure paths). /admin/chapter-onboarding admin list shows "Copy form URL" + "Open form" buttons per invite + the full URL in the detail dialog.
  4. ✅ Per-chapter image scoping on /admin/images — page.tsx builds chapterFilter based on role: SUPER_ADMIN sees all chapters; CHAPTER_ORGANIZER + CO_HOST see only their own chapter (me.chapterId); ADMIN sees chapters in their own country.
  5. ✅ Provisioning endpoint — POST /api/admin/chapter-onboarding/[id]/provision takes a SUBMITTED invite + executes all 6 provisioning steps atomically:
     (a) Country lookup-or-create (slug + best-effort country code + flag emoji)
     (b) Chapter row create (name, slug with uniqueness fallback, countryId, city, timezone, whatsappGroupUrl, linkedinUrl, heroImageUrl from form's landingHeroUrl or chaptercore.md chapterHero default)
     (c) Brand images — re-uploads each form-supplied image to chapter-brand/<chapterId>/ prefix, falls back to chaptercore.md default for any not supplied (favicon → /defaults/chapter-core/favicon.webp, loginHero → /defaults/chapter-core/login-hero.png, loginBanner → /defaults/chapter-core/login-banner.png, emailLogo → /defaults/chapter-core/email-logo.png)
     (d) Lead User linked to new chapter (chapterId + countryId) + upgraded to CHAPTER_ORGANIZER (SUPER_ADMIN/ADMIN never downgraded)
     (e) Email infrastructure cloned from Tel Aviv source chapter (audiences + flows + DRAFT campaigns), idempotent via [cloned-from:<id>] markers
     (f) Invite row updated with appliedChapterId + appliedAt
  6. ✅ "Approve & Provision" button — chapter-onboarding-admin-list.tsx shows the button in the detail dialog when status === SUBMITTED && !appliedChapterId. On success shows "Open chapter admin" link. Idempotent (409 if already provisioned).
  7. ✅ Default email logo fallback — chaptercore.md defines emailLogo → /defaults/chapter-core/email-logo.png (captured from Montreal's original brand assets). Provision route falls back to this for any chapter where the form didn't supply one. Verified the file exists on disk (279KB PNG).
- Ran `npx tsc --noEmit` across the whole project — ZERO errors in any chapter-onboarding-related file. (Pre-existing TS errors in unrelated files: non-member-dashboard.tsx, event-profile-canvas.tsx, image-edit skill, stock-analysis skill — out of scope.)

Stage Summary:
- All 7 tasks (A–F + email-logo-default verification) were already implemented in the prior session before context ran out. This session confirmed the implementation is complete, compiles cleanly, and matches the user's spec.
- The full end-to-end chapter onboarding workflow is ready:
  Super-admin creates invite from /admin/members/[id] → lead receives email + URL → fills 9-section form with image uploads → /admin/chapter-onboarding shows the submission with "Approve & Provision" button → click provisions a real Chapter + ChapterSetting rows + email infra + links lead user.
