 ...
 *
 * Landing page for the AI Salon V7.2 4-Tier Platform Plan PDF.
 *
 * Renders the 43-page PDF inline via <iframe> so the IM preview panel
 * shows actual document content (not just a blank page), and provides
 * a prominent Download button for users who want the file.
 *
 * The PDF itself is served by /api/downloads/4-tier-platform-plan.pdf?inline=1
 * which streams it with Content-Type: application/pdf and
 * Content-Disposition: inline so the browser renders it natively.
 */
export default function FourTierPlatformPlanPage() {
  ...
  useEffect(() => {
    ...
    const bust = Date.now();
    setPdfUrl(`/api/downloads/4-tier-platform-plan.pdf?inline=1&t=${bust}`);
    setDownloadUrl(`/api/downloads/4-tier-platform-plan.pdf?t=${bust}`);
  }, []);
        ...
        <div className="mx-auto max-w-6xl px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div className="text-xs font-semibold uppercase tracking-wider text-[#0066FF]">
              AI Salon · Platform Architecture · V7.2
            </div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              43 pages · Crystal Blue Tech Blueprint · PDF preview rendered inline ·{" "}
              <span className="text-[#FF005A] font-semibold">All 8 design decisions resolved</span>
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <a
              href={downloadUrl}
              download="4-tier-platform-plan.pdf"
      ...
      <section className="mx-auto max-w-6xl px-6 py-6">
          ...
          <div className="h-[80vh] min-h-[600px] bg-slate-200">
            {pdfUrl ? (
              <iframe
                key={iframeKey}
                src={pdfUrl}
                title="4-Tier Platform Plan PDF preview"
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-slate-600">
          ...
          <code className="rounded bg-slate-200 px-1.5 py-0.5">
            /api/downloads/4-tier-platform-plan.pdf
          </code>
          <span className="text-slate-400">·</span>
          <code className="rounded bg-slate-200 px-1.5 py-0.5">
            /api/downloads/4-tier-platform-plan.pdf?inline=1
          </code>
          <span className="text-slate-400">·</span>
          <a href="/3-tier-platform-plan" className="text-[#0066FF] hover:underline">
            ← V7.1 3-tier plan (PDF-2)
          </a>
        </div>
      </section>
    </main>
